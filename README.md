...
**試算表標題列（第一列）：** `Timestamp` | `Class` | `Seat` | `Name` | `BaseScore` | `Score` | `Time`  
...
- 僅從 Google 試算表讀取（`GET ?action=leaderboard&limit=6`），依**防疫積分**（總積分×10＋時間獎勵）排序，同分則時間較短者優先。
...
| 欄位順序不符 | GAS `appendRow` 順序須為：時間戳記、Class、Seat、Name、BaseScore、Score(防疫)、Time（見範例檔） |
